...
export type ChangeValuesFunction = <
   Values extends
      | 'headquarters'
      | 'users'
      | 'clinic'
   ...
   Param extends Object extends any[] ? keyof Object[number] : keyof Object,
>(
   tree: Values,
   param: Param,
   value: Object extends any[] ? Object[number][Param] : Object[Param],
   index: Object extends any[] ? number : null,
) => void;

export default function EditView({ data, headquarters, users }: IEditView) {
   ...
   const [newAdmin, setNewAdmin] = useState<NewUser>();
   ...
   function addHeadquarter() {
      setValues(prev => {
         ...
         aux.headquarters.push({
            address: '',
            city: '',
            index: aux.headquarters.length,
            name: '',
            phone: '',
            removed: false,
         });
   ...
   async function edit() {
      ...
      try {
         ...
         if (newAdmin) {
            ...
            admin_id = user.userSub;

            if (!user) {
               throw Error('Error creando al usuario');
            }
         }
   ...
   return (
      <>
         <CancelConfirmationModal
            isOpen={closingOpen}
            route={SORoutes.management_clinic_slug(values.clinic.slug).details}
         ...
         <ChangeAdminModal
            ...
            isOpen={isChangingAdmin}
            setIsOpen={setIsChangingAdmin}
            selectedAdminId={values.clinic.administrator_id}
            users={users.filter(
               user =>
         ...
         {admin && (
            <section className="mx-24 my-10">
               <UserOverviewCard
                  ...
                  code={code}
                  editing
                  action={() => setIsChangingAdmin(true)}
               />
            </section>
         )}
      </>
            ...
            <Dialog className="flex flex-col items-center gap-9 rounded-xl p-8 sm:p-14">
               ...
               <div>
                  ...
                  <p className="px-14 text-center !text-base !font-normal text-on-background-text body-1">
                     Podrás ver tus cambios actualizados en el detalle de
                     clínica
                  </p>
               </div>
               <Button
